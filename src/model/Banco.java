...
public class Banco {
	...
	public static void validarCnpj(String cnpj) throws ModelException{
		...
		if (cnpj.length() != 18)
			throw new ModelException("O CNPJ deve conter 18 caracteres");
		for (int i = 0; i < 2; i++) {
	...
	public static void validarNome(String nome) throws ModelException {
		if (nome == null || nome.length() == 0)
			throw new ModelException("Campo Vazio!");
		if (nome.length() > 20)
			throw new ModelException(" Limite de 20 caracteres");
		for (int i = 0; i < nome.length(); i++) {
			char c = nome.charAt(i);
			if (!Character.isAlphabetic(c) && !Character.isSpaceChar(c))
				throw new ModelException("O nome não dever conter números.");
		}
	}
	...
	public String toString() {
		return "CNPJ: " + this.cnpj + ";" + "Nome: " + ";" + this.nome + ";" + "Código: " + this.codigo;
	}
}
